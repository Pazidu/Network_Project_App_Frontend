...
// --- 1. CUSTOM SPEEDOMETER COMPONENT ---
...
export default function NetworkDetailsScreen({ navigation }) {
  ...
  // 1. Fetch Local Network Info (Internal IP, Gateway, SSID)
  const loadNetworkInfo = async () => {
    await requestPermissions();
    try {
      // Run internal checks in parallel
      const [state, ip, gateway, ssid] = await Promise.all([
        NetInfo.fetch(),
        NetworkInfo.getIPV4Address().catch(() => 'Unknown'),
        NetworkInfo.getGatewayIPAddress().catch(() => 'Unknown'),
        NetworkInfo.getSSID().catch(() => null),
      ]);
  ...
  // 2. Fetch ISP independently (Using ipwho.is)
  const fetchIsp = async () => {
    try {
      // Using ipwho.is as it's more permissive for mobile apps than ipapi.co
      const response = await fetch('https://ipwho.is/');
      const data = await response.json();
      
      if (data.success) {
        setIsp(data.connection?.isp || data.connection?.org || 'Unknown ISP');
      } else {
        setIsp('Unknown ISP');
      }
    } catch (e) {
      console.log('ISP Fetch Error:', e);
      setIsp('Unknown ISP'); // Fallback so it doesn't stay "Fetching..."
    }
  };

  // --- ANIMATED SPEED TEST LOGIC ---
  const runSpeedTest = async () => {
    ...
    try {
      ...
      const simInterval = setInterval(() => {
        const randomFluctuation = Math.random() * 20 + 5; 
      ...
      const uploadEst = speedMbps * 0.35; 
  ...
  // --- EFFECTS ---
  useEffect(() => {
    // 1. Initial Load - Call all independently
    loadNetworkInfo();
    fetchIsp(); 
    checkPing();

    // 2. Refresh Ping periodically
  ...
  }, [info, ping, downloadSpeed, uploadSpeed, isp]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.refreshButton} onPress={() => { loadNetworkInfo(); fetchIsp(); }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.detailsList}>
          ...
          {/* <--- ISP FEATURE (UPDATED) ---> */}
          <DetailRow icon="earth" label="ISP" value={isp} color="#3b82f6" />
